...
const helpers = {
  ...
  thresholdToZero: function thresholdToZero (m) {
    const me = m.elements;
    for (let i = 0; i < 9; ++i) {
      if (Utils.isZero(me[i], EPSILON)) {
        me[i] = 0;
      }
    }
  },
  ...
  findRank: function findRank (m) {
    const test = helpers.rrefInPlace(m.clone());
    let rank = 0;
    for (let i = 0; i < 3; ++i) {
      if (helpers.isRowNonzero(test, i)) {
        rank++;
      }
    }
    return rank;
